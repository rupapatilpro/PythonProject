...
public class ParameterizationUsingExcel {
    ...
    // Method 	getDataFromExcel is defined in class Util
    	String[][] testData = UtilExcelData.getDataFromExcel(UtilExcelData.FILE_PATH,
    			UtilExcelData.SHEET_NAME, UtilExcelData.TABLE_NAME);
	...
	for (int i = 0; i < testData.length; i++) {
	    ...
	    try{ 
			...
			alt.accept();
			if (actualBoxtitle.contains(UtilExcelData.EXPECT_ERROR)) { // Compare Error Text with Expected Error Value
			...
			if (actualTitle.contains(UtilExcelData.EXPECT_TITLE)) {
			    System.out.println("Test case SS[" + i + "]: Passed");
			} else {
			    System.out.println("Test case SS[" + i + "]: Failed");
			}
